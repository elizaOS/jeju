...
contract ReputationRegistryTest is Test {
    ...
    function testGiveFeedback() public {
        bytes memory feedbackAuth = createFeedbackAuth(
            serviceId,
            client1,
            10,
    ...
    function testRevokeFeedback() public {
        ...
        (uint64 count, ) = reputationRegistry.getSummary(
            serviceId,
            noClients,
            bytes32(0),
            bytes32(0)
    ...
    function testAppendResponse() public {
        ...
        uint64 responseCount = reputationRegistry.getResponseCount(
            serviceId,
            client1,
            1,
            responders
    ...
    function testTEEServiceReputation() public {
        ...
        vm.prank(client1);
        reputationRegistry.giveFeedback(teeServiceId, 100, bytes32("tee-verified"), bytes32("sgx"), "", bytes32(0), auth1);
        
        vm.prank(client2);
        reputationRegistry.giveFeedback(teeServiceId, 98, bytes32("tee-verified"), bytes32("sgx"), "", bytes32(0), auth2);
        
        vm.prank(client3);
        reputationRegistry.giveFeedback(teeServiceId, 100, bytes32("tee-verified"), bytes32("sgx"), "", bytes32(0), auth3);
    ...
    function testCompleteServiceReputationLifecycle() public {
        ...
        bytes memory auth1 = createFeedbackAuth(serviceId, client1, 10, block.timestamp + 1 days);
        vm.prank(client1);
        reputationRegistry.giveFeedback(serviceId, 95, bytes32("quality"), bytes32(0), "ipfs://review1", keccak256("data1"), auth1);
        ...
        bytes memory auth2 = createFeedbackAuth(serviceId, client2, 10, block.timestamp + 1 days);
        vm.prank(client2);
        reputationRegistry.giveFeedback(serviceId, 90, bytes32("quality"), bytes32(0), "ipfs://review2", keccak256("data2"), auth2);
        
        // 4. One client has issue and gives low score
        bytes memory auth3 = createFeedbackAuth(serviceId, client3, 10, block.timestamp + 1 days);
        vm.prank(client3);
        reputationRegistry.giveFeedback(serviceId, 60, bytes32("issue"), bytes32("downtime"), "ipfs://complaint", keccak256("data3"), auth3);
        ...
        vm.prank(client3);
        reputationRegistry.giveFeedback(serviceId, 85, bytes32("improved"), bytes32("fixed"), "ipfs://updated", keccak256("data4"), auth3);
