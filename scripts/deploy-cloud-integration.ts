 ...
 * - CloudPaymaster (enhanced paymaster)
...
async function main() {
  ...
  try {
    ...
    console.log('📋 Deploying CloudServiceRegistry...');
    summary.cloudServiceRegistry = await deployCloudServiceRegistry();
    totalGasUsed += BigInt(summary.cloudServiceRegistry.gasUsed);
    console.log(`✅ CloudServiceRegistry: ${summary.cloudServiceRegistry.address}\n`);

    // 2. Deploy CloudPaymaster
    console.log('💳 Deploying CloudPaymaster...');
    summary.cloudPaymaster = await deployCloudPaymaster(summary.cloudServiceRegistry.address);
    totalGasUsed += BigInt(summary.cloudPaymaster.gasUsed);
    console.log(`✅ CloudPaymaster: ${summary.cloudPaymaster.address}\n`);
...
async function deployCloudServiceRegistry(): Promise<DeploymentResult> {
  // Use Foundry to deploy CloudServiceRegistry
  const { execSync } = await import('child_process');

  try {
    const deployCmd = `forge create src/cloud/CloudServiceRegistry.sol:CloudServiceRegistry \
    ...
    logger.error('Failed to deploy CloudServiceRegistry', error);
    throw new Error(`CloudServiceRegistry deployment failed: ${error.message}`);
  }
}

async function deployCloudPaymaster(serviceRegistryAddress: string): Promise<DeploymentResult> {
  // Deploy CloudPaymaster with dependency on CloudServiceRegistry
  const { execSync } = await import('child_process');

  try {
    // CloudPaymaster needs: serviceRegistry, entryPoint, elizaOSToken, depositPercentage, treasury
    ...
    const deployCmd = `forge create src/paymaster/CloudPaymaster.sol:CloudPaymaster \
      --rpc-url ${process.env.JEJU_RPC_URL} \
      --private-key ${process.env.DEPLOYER_PRIVATE_KEY} \
    ...
    logger.error('Failed to deploy CloudPaymaster', error);
    throw new Error(`CloudPaymaster deployment failed: ${error.message}`);
  }
}
...
async function deployCreditPurchaseContract(): Promise<DeploymentResult> {
  ...
  try {
    ...
    const deployCmd = `forge create src/cloud/CreditPurchaseContract.sol:CreditPurchaseContract \
      --rpc-url ${process.env.JEJU_RPC_URL} \
      --private-key ${process.env.DEPLOYER_PRIVATE_KEY} \
...
async function setupConfiguration(summary: DeploymentSummary) {
  ...
  // CloudServiceRegistry already initializes: chat-completion, image-generation, video-generation, container

  console.log('  🔗 Authorizing CloudPaymaster as caller in service registry...');
  try {
    ...
    console.log('    ✅ CloudPaymaster authorized');
  } catch (error) {
