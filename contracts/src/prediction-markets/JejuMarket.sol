 ...
 */
contract JejuMarket is ReentrancyGuard, Pausable, Ownable {
    struct Market {
        bytes32 sessionId;
        string question;
        uint256 yesShares;
     ...
     * @param elizaOSAmount Amount of elizaOS to spend
    ...
    function buy(
        ...
        shares = calculateSharesReceived(sessionId, outcome, elizaOSAmount);
        if (shares < minShares) revert SlippageTooHigh();

        // Transfer elizaOS from user
        require(elizaOS.transferFrom(msg.sender, address(this), elizaOSAmount), "Transfer failed");
        ...
        market.totalVolume += elizaOSAmount;
        ...
        position.totalSpent += elizaOSAmount;

        emit SharesPurchased(sessionId, msg.sender, outcome, shares, elizaOSAmount);
    }
    ...
    function sell(
        ...
        // Transfer payout
        require(elizaOS.transfer(msg.sender, payout), "Transfer failed");

        emit SharesSold(sessionId, msg.sender, outcome, shareAmount, payout);
    }
    ...
    function claimPayout(bytes32 sessionId) external nonReentrant returns (uint256 payout) {
        ...
        uint256 totalPool = elizaOS.balanceOf(address(this));
        ...
        require(elizaOS.transfer(treasury, platformFeeAmount), "Transfer failed");
