...
contract NodeStakingFuzzTest is Test {
    ...
    function setUp() public {
        ...
        staking =
            new NodeStakingManager(address(registry), address(factory), address(oracle), oracleAddr, address(this));
    ...
    function testFuzz_RegisterWithVariousAmounts(uint256 amount) public {
        ...
        bytes32 nodeId = staking.registerNode(
            address(token), amount, address(token), "https://fuzz-test.com", INodeStakingManager.Region.NorthAmerica
        );
    ...
    function testFuzz_TimeElapsed(uint32 timeElapsedDays) public {
        uint256 timeElapsed = bound(timeElapsedDays, 1, 365) * 1 days;
