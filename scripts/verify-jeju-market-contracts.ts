#!/usr/bin/env bun
/**
 * Verify JejuMarket Contract Deployment
 *
 * This script performs comprehensive verification of the JejuMarket system:
 ...
 *   NETWORK=mainnet bun run scripts/verify-jeju-market-contracts.ts
 */
...
function loadDeploymentConfig(): DeploymentConfig {
    ...
    const deploymentFile = join(process.cwd(), 'deployments', `jeju-market-${network}.json`);
...
// ============================================================================
// JejuMarket Contract Verification
// ============================================================================

async function verifyJejuMarket(config: DeploymentConfig) {
    section('JejuMarket Contract Verification');

    const marketAddress = config.contracts.jejuMarket;
    if (!marketAddress) {
        addResult('Market', 'JejuMarket', 'SKIP', 'JejuMarket not deployed');
        return;
    }
...
async function verifyPermissions(config: DeploymentConfig) {
    ...
    const contracts = [
        ...
        { name: 'JejuMarket', address: config.contracts.jejuMarket },
...
async function testMarketCreation(config: DeploymentConfig) {
    ...
    const marketAddress = config.contracts.jejuMarket;
    if (!marketAddress) {
        addResult('Functionality', 'Market Creation', 'SKIP', 'JejuMarket not deployed');
        return;
    }
...
async function testBetting(config: DeploymentConfig) {
    ...
    const marketAddress = config.contracts.jejuMarket;
    if (!marketAddress) {
        addResult('Functionality', 'Betting', 'SKIP', 'JejuMarket not deployed');
        return;
    }
...
function printSummary(report: VerificationReport) {
    ...
    if (report.summary.skipped > 0) {
        ...
        console.log('\nSee documentation/deployment/jeju-market-complete-deployment.md for testing procedures.\n');
    }
}
