...
contract ModerationIntegrationTest is Test {
    ...
    function setUp() public {
        ...
        governance = new RegistryGovernance(
            payable(address(registry)), address(mockPredimarket), admin, RegistryGovernance.Environment.LOCALNET, admin
        );

        banManager = new BanManager(address(governance), admin);
        labelManager =
            new ReputationLabelManager(address(banManager), address(mockPredimarket), address(governance), admin);
...
contract MockPredimarket {
    ...
    function createMarket(bytes32, string memory, uint256) external view {
