...
library ItemLib {
    function checkItemRequirements(
        uint8 playerAttackLevel,
        uint8 playerStrengthLevel,
        uint8 playerDefenseLevel,
        uint8 playerRangedLevel,
        uint8 requiredAttackLevel,
        uint8 requiredStrengthLevel,
        uint8 requiredDefenseLevel,
        uint8 requiredRangedLevel
    ) internal pure returns (bool meetsRequirements, string memory missingRequirement) {
    ...
    function getEquipmentSlot(uint8 itemType, string memory itemName)
        internal
        pure
        returns (uint8 slot, bool isEquipment)
    {
        ...
        if (itemType == 1) {
            ...
            if (_contains(bytes(itemName), bytes("body")) || _contains(bytes(itemName), bytes("Plate"))) {
                return (3, true);
            }
