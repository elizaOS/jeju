...
const VALID_PERIODS: LeaderboardPeriod[] = ["all", "weekly", "monthly"];

function isValidPeriod(value: string | null): value is LeaderboardPeriod {
  return value !== null && VALID_PERIODS.includes(value as LeaderboardPeriod);
}

function parsePeriod(value: string | null): LeaderboardPeriod {
  return isValidPeriod(value) ? value : "all";
}
...
export function Leaderboard({ tabs, tags }: LeaderboardProps) {
  ...
  useEffect(() => {
    setCurrentPeriod(parsePeriod(searchParams.get("period")));
    setCurrentPage(1);
