...
export class AgentRuntime implements IAgentRuntime {
  ...
  async initialize(): Promise<void> {
    ...
    if (!existingAgent) {
      throw new Error(`Agent ${this.character.name} does not exist in database after ensureAgentExists call`);
    }
    ...
    if (!agentEntity) {
      ...
      if (!agentEntity) {
        throw new Error(`Entity not found after creation for ${this.agentId}`);
      }
    ...
    if (!participants.includes(this.agentEntityId)) {
      const added = await this.addParticipant(this.agentEntityId, this.agentId);
      if (!added) {
        throw new Error(`Failed to add agent ${this.agentId} with entity ${this.agentEntityId} as participant to its own room`);
      }
      this.logger.debug(
        `Agent ${this.character.name} (entity: ${this.agentEntityId}) linked to its own room successfully`
      );
    }
  ...
  async sendMessageToTarget(target: TargetInfo, content: Content): Promise<void> {
    ...
    if (!handler) {
      throw new Error(`No send handler registered for source: ${target.source}`);
    }
