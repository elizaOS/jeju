...
export class CouncilMCPServer {
  ...
  private async readResource(uri: string): Promise<Record<string, unknown> | null> {
    switch (uri) {
      case 'council://proposals/active': return this.blockchain.listProposals(true, 50);
      case 'council://proposals/all': return this.blockchain.listProposals(false, 100);
      case 'council://ceo/status': return this.blockchain.getCEOStatus();
      case 'council://governance/stats': return this.blockchain.getGovernanceStats();
      case 'council://council/agents': return this.blockchain.getCouncilStatus();
      default: return null;
    }
  }
  ...
  private async callTool(name: string, args: Record<string, string>): Promise<{ result: Record<string, unknown>; isError: boolean }> {
    switch (name) {
      ...
      case 'get_council_votes': return this.getCouncilVotes(args.proposalId);
