...
contract InferenceRegistry is Ownable, ReentrancyGuard {
     ...
     */
    function updateEndpointHealth(string calldata modelId, uint256 index, bool healthy, uint256 latencyMs) external {
        ModelEndpoint[] storage endpoints = modelEndpoints[modelId];
        require(index < endpoints.length, "Invalid index");
     ...
     */
    function getBestEndpoint(string calldata modelId)
        external
        view
        returns (address providerAddress, string memory endpoint, TEEType teeType, uint256 currentLoad)
    {
        ModelEndpoint[] storage endpoints = modelEndpoints[modelId];
     ...
     */
    function estimateCost(string calldata modelId, uint256 inputTokens, uint256 outputTokens)
        external
        view
        returns (uint256)
    {
