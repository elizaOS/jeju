...
contract TokenRegistrySecurityTest is Test {
    ...
    function test_RejectFeeOnTransferToken() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(TokenRegistry.FeeOnTransferToken.selector, address(feeToken)));

        registry.registerToken{value: 0.1 ether}(address(feeToken), address(oracle), 0, 200);
    }
    ...
    function test_AcceptNormalToken() public {
        ...
        // Should succeed
        uint256 tokenId = registry.registerToken{value: 0.1 ether}(address(goodToken), address(oracle), 0, 200);
    ...
    function test_RejectZeroAddressOracle() public {
        ...
        registry.registerToken{value: 0.1 ether}(address(goodToken), address(0), 0, 200);
    }
    ...
    function test_RegistrationFeePreventsSpam() public {
        ...
        vm.prank(attacker);
        vm.expectRevert(abi.encodeWithSelector(TokenRegistry.InsufficientRegistrationFee.selector, 0.1 ether, 0));

        registry.registerToken(address(goodToken), address(oracle), 0, 200);
    }
    ...
    function test_RegistrationFeeGoesToTreasury() public {
        ...
        vm.prank(user1);
        registry.registerToken{value: 0.1 ether}(address(goodToken), address(oracle), 0, 200);
