...
library SqrtPriceMath {
        ...
        returns (uint160)
    {
        ...
        if (add) {
            ...
            unchecked {
                uint256 product;
                if (!((product = amount * sqrtPX96) / amount == sqrtPX96)) revert InvalidPriceOrLiquidity();
                if (!(numerator1 > product)) revert NotEnoughLiquidity();
                uint256 denominator = numerator1 - product;
                return uint160(FullMath.mulDivRoundingUp(numerator1, sqrtPX96, denominator));
            }
        }
        ...
        if (add) {
            ...
            uint256 quotient = (
                amount <= type(uint160).max
                    ? UnsafeMath.divRoundingUp(amount << 96, liquidity)
