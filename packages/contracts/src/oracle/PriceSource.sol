...
/**
 * @title PriceSource
 * @notice Reads prices on Ethereum and relays them to Jeju via CrossDomainMessenger
 * @dev Deploy this contract on Ethereum
 *
 * Architecture:
 * 1. Reads ETH/USD from Chainlink on Ethereum
 * 2. Reads elizaOS/ETH from Uniswap V3 on Ethereum
...
contract PriceSource is Ownable, Pausable {
    ...
    address public constant CHAINLINK_ETH_USD = 0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419; // Ethereum mainnet

    address public constant UNISWAP_V3_FACTORY = 0x1F98431c8aD98523631AE4a59f267346ea31F984; // Ethereum mainnet
    ...
    event PricesRelayed(uint256 ethPrice, uint256 elizaPrice, uint256 timestamp, bytes32 messageHash, uint256 nonce);
    ...
    constructor(address _elizaOSToken, address _crossChainRelayOnJeju, address _priceUpdater, address initialOwner)
    ...
    function updateAndRelay() external whenNotPaused {
        ...
        // 1. Read ETH/USD from Chainlink (view-only)
        uint256 ethPrice = _readChainlink();

        // 2. Read elizaOS/USD from Uniswap (view-only)
        uint256 elizaPrice = _readUniswapV3();

        // 3. Validate prices
        _validatePrices(ethPrice, elizaPrice);

        // 4. EFFECTS: Update state BEFORE external calls (CEI pattern)
        lastETHPrice = ethPrice;
        lastElizaPrice = elizaPrice;
        lastUpdateTime = block.timestamp;
        updateCount++;

        // Track nonce before external call
        uint256 currentNonce = messageNonce;
        messageNonce++;

        // 5. Prepare message
        bytes memory message = abi.encodeWithSignature(
            "receivePriceUpdate(uint256,uint256,uint256)", ethPrice, elizaPrice, block.timestamp
        );

        // 6. INTERACTIONS: Call CrossDomainMessenger last
        ...
        bytes32 messageHash = returnData.length >= 32 ? abi.decode(returnData, (bytes32)) : bytes32(0);
    ...
    function _readChainlink() internal view returns (uint256 ethPrice) {
        ...
        (bool pauseSuccess, bytes memory pauseData) = CHAINLINK_ETH_USD.staticcall(abi.encodeWithSignature("paused()"));
        ...
        (bool success, bytes memory data) = CHAINLINK_ETH_USD.staticcall(abi.encodeWithSignature("latestRoundData()"));
        ...
        returns (address pool)
    {
        (bool success, bytes memory data) =
            factory.staticcall(abi.encodeWithSignature("getPool(address,address,uint24)", tokenA, tokenB, fee));
    ...
    function _getPoolPrice(address pool) internal view returns (uint256 price) {
        ...
        // Get token order
        (bool success0, bytes memory data0) = pool.staticcall(abi.encodeWithSignature("token0()"));
        ...
        // Get token1 address
        (bool success1, bytes memory data1) = pool.staticcall(abi.encodeWithSignature("token1()"));
     ...
     */
    function _mulDivQ192(uint256 numerator, uint256 denominator, uint8 token0Decimals, uint8 token1Decimals)
        internal
        pure
    ...
    function getLastPrices() external view returns (uint256 ethPrice, uint256 elizaPrice, uint256 timestamp) {
     ...
     */
    function previewPrices() external view returns (uint256 ethPrice, uint256 elizaPrice) {
