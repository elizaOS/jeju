 ...
 * @notice Receives price updates from Ethereum via CrossDomainMessenger
 * @dev Deploy on Jeju. Works with PriceSource.sol deployed on Ethereum.
 *
 * Architecture:
 * 1. PriceSource on Ethereum reads Chainlink + DEX prices
...
contract CrossChainPriceRelay is Ownable, ReentrancyGuard, Pausable {
    ...
    /// @notice Address of PriceSource contract on Ethereum
    address public priceSourceOnEthereum;
    ...
    event PriceReceived(uint256 ethPrice, uint256 elizaPrice, uint256 sourceTimestamp, uint256 relayTimestamp);
    ...
    constructor(address _priceSourceOnEthereum, address _targetOracle, address initialOwner) Ownable(initialOwner) {
        priceSourceOnEthereum = _priceSourceOnEthereum;
    ...
    /**
     * @notice Receive price update from Ethereum via CrossDomainMessenger
     * @param ethPrice ETH/USD price (8 decimals)
     * @param elizaPrice elizaOS/USD price (8 decimals)
     * @param sourceTimestamp Timestamp when prices were read on Ethereum
     ...
     */
    function receivePriceUpdate(uint256 ethPrice, uint256 elizaPrice, uint256 sourceTimestamp)
        external
        nonReentrant
        whenNotPaused
    {
        ...
        // Verify original sender is our authorized PriceSource on Ethereum
        address xDomainSender = ICrossDomainMessenger(msg.sender).xDomainMessageSender();

        if (xDomainSender != priceSourceOnEthereum) {
        ...
        (bool success, bytes memory returnData) =
            targetOracle.call(abi.encodeWithSignature("updatePrices(uint256,uint256)", ethPrice, elizaPrice));
        ...
        (bool checkSuccess, bytes memory checkData) =
            targetOracle.staticcall(abi.encodeWithSignature("lastUpdateTime()"));
    ...
    function getRelayInfo()
        ...
        view
        returns (
            address _priceSourceOnEthereum,
    ...
    {
        return
            (priceSourceOnEthereum, targetOracle, lastUpdateTime, updateCount, lastETHPrice, lastElizaPrice, paused());
    }

    /**
     * @notice Get last received prices
     */
    function getLastPrices() external view returns (uint256 _ethPrice, uint256 _elizaPrice, uint256 _timestamp) {
